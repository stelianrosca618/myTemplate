                ...
                <div class="nk-pps-data">
                    <ul class="nk-olist">
                        <li class="nk-olist-item">
                            ...
                            <div class="data"><span class="method"><em class="icon ni ni-coins"></em> <span>{{ data_get($paymentMethodDetails, 'name') }}</span></span></div>
                        </li>
                    ...
                    <div class="sub-text-sm">{!! __('* You will be redirect to :gateway website once you confirm.', ['gateway' => '<strong class="text-dark">' . "Coinbase" . '</strong>']) !!}</div>
                </div>
                <div class="nk-pps-field form-action text-center">
                    <div class="nk-pps-action">
                        <a href="javascript:void(0)" class="btn btn-lg btn-block btn-primary" id="make-coinbase-payment">
...
<script>
!(function (App, $) {
    $(document).on('click', '#make-coinbase-payment', function() {
        let $self = $(this), data = {id: "{{ the_hash($transaction->id) }}"}, url = "{{ route('user.gateway.make-payment.coinbase') }}";
        if(url) {
            App.Form.toPost(url, data, { 
                btn: $self, onSuccess: function(res) { if (res.redirect) { window.location.href = res.approve_url.href; } } 
            });
        }
    });
})(NioApp, jQuery);

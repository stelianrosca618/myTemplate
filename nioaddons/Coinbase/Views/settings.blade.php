...
@section('title', __('Coinbase Commerce - Payment Method'))
        ...
        <div class="nk-block card card-bordered nk-block-mh">
            <div class="card-inner">
                <div class="justify-between">
                    <h5 class="title">
                        {{ __('Coinbase Commerce') }}
                        <span class="meta ml-1">
                            <span class="badge badge-pill badge-danger nk-tooltip"{!! (is_demo()) ? ' title="This add-on is NOT part of the main package."' : '' !!}>{{ __('Add-ons') }}</span>
                        </span>
                        @if ($outdated)
                        <span class="meta ml-1">
                ...
                <p>{{ __('Accept crypto related payments via Coinbase Commerce account.') }}</p>
                <div class="divider"></div>
                <div class="row">
                    <div class="col-12">
                        <form action="{{ route('admin.settings.gateway.payment.coinbase.save') }}" class="form-settings" method="POST" autocomplete="off">
                            <div class="row g-5">
                                        ...
                                        <div class="col-12 col-xxl-6">
                                            <div class="form-group">
                                                ...
                                                <div class="form-control-wrap">
                                                    <input type="text" name="name" class="form-control" value="{{ data_get($settings, 'name', __('Coinbase')) }}">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-12 col-xxl-6">
                                            <div class="form-group">
                                                ...
                                                <div class="form-control-group">
                                                    <input type="text" name="desc" value="{{ data_get($settings, 'desc', __('Pay securely with Coinbase payment gateway.')) }}" class="form-control">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-12">
                                    ...
                                    <div class="row gy-3">
                                        <div class="col-12">
                                            <div class="form-group">
                                                ...
                                                <div class="form-control-group">
                                                    <input type="text" name="config[api][key]" value="{{ data_get($settings, 'config.api.key') }}" class="form-control">
                                                </div>
                                                <div class="form-note">{!! __('Coinbase Commerce account API key.') !!}</div>
                                            </div>
                                        </div>
                                        <div class="col-12">
                                            <div class="form-group">
                                                <label class="form-label">{{ __('Webhook Shared Secret') }}</label>
                                                <div class="form-control-group">
                                                    <input type="text" name="config[api][webhook]" value="{{ data_get($settings, 'config.api.webhook') }}" class="form-control">
                                                </div>
                                                <div class="form-note">{!! __('For automatically payment verification.') !!}</div>
                                            </div>
                                        </div>
                                        <div class="col-12">
                                            <div class="form-group">
                                        ...
                                        <div class="col-12">
                                            <div class="note mt-2 pl-2 border-left border-primary">
                                                <p class="mb-1"><strong>{{ __('Please Note:') }}</strong> {!! __("You must setup webhook into Coinbase Commerce :setting page.", ['setting' => '<a class="link" href="https://commerce.coinbase.com/dashboard/settings" target="_blank">' . strtolower(__('Settings')) .'</a>']) !!}</p>

                                                <p class="mb-1">{!! __('Webhook subscription endpoint: :path', ['path' => '<br><code>' . route('public.payment.coinbase.confirm')]) . '</code>' !!}</p>
                                                <p class="mb-1">{!! __('Must enable the event of :code and disable other events.', ['code' => '<code class="code-fnc">charge:confirmed</code>']) !!}</p>
                                                <p>{!! __("Ensure your application works over :port as Coinbase required :port for webhook.", ['port' => '<code>'.'https'.'</code>']) !!}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>

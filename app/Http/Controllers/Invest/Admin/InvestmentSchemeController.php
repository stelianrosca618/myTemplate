...
class InvestmentSchemeController extends Controller
{
    ...
    public function variableSchemeList(Request $request, $type = null){
        if (!Schema::hasColumns('iv_schemes', ['deleted_at', 'type'])) {
            return back()->with(['notice' => __("To continue, please install the update and migrate the application database.")]);
        }


        $schemes = IvScheme::all();
        dd($schemes);
        $status = 'varaible';
    ...
    {
        if (!Schema::hasColumns('iv_schemes', ['deleted_at'])) {
            return back()->with(['notice' => __("To continue, please install the update and migrate the application database.")]);
        }

       if($status == 'variable'){
            $schemes = IvScheme::where('type', 'variable');   
         
        }else{
            $schemes = IvScheme::query()->withoutGlobalScope('exceptArchived')->whereNull('type');    
        ...
        if ($status) {
            if($status != 'variable'){
                $schemes->where('status', $status);
            }
        } else {
            $schemes->where('status', SchemeStatus::ACTIVE);
        }
    ...
    public function actionScheme(Request $request, $action=null)
    {
        ...
        if (!in_array($type, ['new', 'edit', 'newvariable', 'editvariable'])) {
    ...
    public function updateVariableScheme(Request $request, $id=null){

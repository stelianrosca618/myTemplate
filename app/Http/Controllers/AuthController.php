...
class AuthController extends Controller
{
    ...
    public function registerForm()
    {
        ...
        $refer_meta = $this->referralService->getReferMeta();
        $refer_by = data_get($refer_meta, 'by', null);
        $refer_code = data_get($refer_meta, 'code', null);
        $refer_user = ($refer_by) ? User::where('id', $refer_by)->first() : '';

        if (disabled_signup() || (sys_settings('signup_allow', 'enable') == 'invite' && empty($refer_by))) {
        ...
        return view('auth.register', compact('user_counts', 'countries', 'refer_user', 'refer_code'));
    }
    ...
    public function register(RegistrationRequest $request)
    {
        ...
        if (disabled_signup() || (sys_settings('signup_allow', 'enable') == 'invite' && empty($refer_by))) {
            throw ValidationException::withMessages(['notice' => __('New registration is not allowed. Please feel free to contact us for more information.')]);
        }
        ...
        try {
            ...
            DB::rollBack();
            $message = __('Sorry, due to technical issues we unable to proceed. Please try again after sometimes or contact us.');

            if ($e->getMessage() == 'invalid-user') {
                $message = __('An error occurred during registration, please try again later. If the issues continues, contact us.');
            }
    ...
    public function confirm(Request $request)
    {
        ...
        return view('auth.confirm', ['email' => $email]);
    }
    ...
    public function verifyCode(Request $request)
    {
        ...
        if ($user && $code) {
            $verifyCode = VerifyToken::where(['code' => $code, 'email' => $user->email])->first();

            if (empty($verifyCode)) {
                return redirect()->route('auth.email.verification')->withErrors(['error' => __('Your verification code does not match or invalid.')]);
            }
            ...
            if (!empty($user)) {
                if (Carbon::now()->diffInMinutes($verifyCode->updated_at) > 30) {
                    return $this->generateVerifyCode($user, 'auth.email.verification');
                }

                return $this->verifyUserEmailByCode($user, $verifyCode);
            }

            return redirect()->route('auth.login.form')->withErrors([
                'error' => __('Email verification failed! The verification code does not match or invalid. Contact us, if you are still having trouble verifying your email.')
            ]);
        }

        return redirect()->route('auth.email.verification')->withErrors(['error' => __('The verfication code is invalid. Please try again.')]);
    }
    ...
    public function confirmCode(Request $request)
    {
        ...
        if ($email && $code) {
            $verifyCode = VerifyToken::where(['code' => $code, 'email' => $email])->first();

            if (empty($verifyCode)) {
                return redirect()->route('auth.confirm')->with(['email' => $email])->withErrors(['error' => __('Your verification code does not match or invalid.')]);
            }
            ...
            if (!empty($user)) {
                if (Carbon::now()->diffInMinutes($verifyCode->updated_at) > 30) {
                    return $this->generateVerifyCode($user);
                }
        ...
        return redirect()->route('auth.confirm')->with(['email' => $email])->withErrors(['error' => __('The verfication code is invalid. Please try again.')]);
    }

    /**
     * @param model $user
     * @param model $verifyCode
     * @return mixed
     * @version 1.0.0
     * @since 1.4.0
     */
    private function verifyUserEmailByCode($user, $verifyCode)
    {
        return $this->wrapInTransaction(function ($user, $verifyCode) {
    ...
    /**
     * @param model $user
     * @param $route name of route, default auth.confirm
     * @return \Illuminate\Contracts\Foundation\Application|\Illuminate\Contracts\View\Factory|\Illuminate\Http\RedirectResponse|\Illuminate\View\View
     * @version 1.0.0
     * @since 1.4.0
     */
    private function generateVerifyCode($user, $route = 'auth.confirm')
    {
        ...
        try {
            ...
            ProcessEmail::dispatch('users-confirm-email', $user);
            $messages['warning'] = __('Sorry, the verification code has been expired! We have sent new verification code to your email.');
        } catch (\Exception $e) {
            save_mailer_log($e, 'users-confirm-email');
            $messages['warning'] = __('Sorry, the verification code has been expired. We are unable to send you new email at the moment. Please contact us via email (:mail) if the problem persist.', ['mail' => sys_settings('site_email')]);
        }
    ...
    public function resendVerifyEmail(Request $request)
    {
        ...
        if (empty($action)) {
            return response()->json(['type' => 'error', 'msg' => __('Sorry, unable to proceed!')]);
        }

        if (!empty($mail) && $action == 'confirm') {
        ...
        if (empty($user)) {
            return redirect()->route('auth.login.form')->withErrors(['error' => __('Please login to your account and resend the verification email again.')]);
        }
        ...
        try {
            ...
            save_mailer_log($e, 'resend-users-confirm-email');
            return response()->json(['type' => 'warning', 'msg' => __('We are unable to send the verification mail at the moment. Please contact us via email at :mail to resolved.', ['mail' => sys_settings('site_email')])]);
        }
    ...
    public function updateEmailAndVerify(Request $request)
    {
        ...
        try {
            $this->auth->generateNewToken($user);
            ProcessEmail::dispatch('users-confirm-email', $user, null, null, null, true);
            session()->flash('mail_sent_success', __('We sent you a confirmation mail to your email. Please check your inbox and confirm.'));
        } catch (\Exception $e) {
            $user->update(['email' => $current_email]);
            save_mailer_log($e, 'resend-users-confirm-email');
    ...
    public function referral(Request $request, ReferralService $referral)
    {
        ...
        if (sys_settings('signup_allow', 'enable') != 'invite') {

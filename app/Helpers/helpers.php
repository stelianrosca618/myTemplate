...
if (!function_exists('random_hash')) {
    /**
     * Generate a token with sha1 encoding
     * @param $str
     * @version 1.0.0
     * @since 1.0
     */
...
if (!function_exists('user_avatar_str')) {
    /**
     * @param $user
     * @param $count
     * @return string
     ...
     * @since 1.3.2
     */
    function user_avatar_str($str, $count = 2)
    {
        return Str::limit(Str::upper($str), $count, '');
    }
}
...
if (!function_exists('trim_input')) {
    /**
     * Strip & trim input, replace multiple spaces inside input with replacement.
     *
     * @param  string  $str
     ...
     * @since  1.3.2
     */
    function trim_input($str, $replacement = '-')
    {
        $str = preg_replace('/\s+/', $replacement, trim(strip_tags($str)));
...
if (!function_exists('logo_link')) {
     ...
     * @since 1.3.2
     */
    function logo_link($type = 'pb', $output = 'link') {
        $links_map = [];

        if (!empty(gss('logo_url_pb'))) {
            $links_map['pb'] = ['url' => gss('logo_url_pb'), 'target' => gss('logo_url_pb_tg', 'off')];
        }

        if (!empty(gss('logo_url_ud'))) {
            $links_map['ud'] = ['url' => gss('logo_url_ud'), 'target' => gss('logo_url_ud_tg', 'off')];
        }

        if ($output == 'raw') {
            return data_get($links_map, $type, []);
        }

        return data_get($links_map, $type.'.url', '');
    }
}
...
if (!function_exists('currency_symbol')) {
     ...
     * @since 1.3.2
     */
    function currency_symbol($code, $suffix = null)
    {
        return ($code != 'XRP' && get_symbol($code) != 'ni-sign-dollar') ? get_symbol($code) . $suffix : get_symbol($code);
    }
}

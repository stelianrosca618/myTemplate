                            ...
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right">
                                    <ul class="link-list-opt no-bdr">
                                        <li><a href="javascript:void(0)" class="actn-scivs" data-action="edit" data-view="modal" data-backdrop="static" data-uid="{{ the_hash($scheme->id) }}"><em class="icon ni ni-edit-fill"></em><span>{{ __('Update Scheme') }}</span></a></li>
                                        @if(data_get($scheme, 'status') != SStatus::ACTIVE)
                                        <li><a href="javascript:void(0)" class="actn-upivs" data-action="{{ SStatus::ACTIVE }}" data-uid="{{ the_hash($scheme->id) }}"><em class="icon ni ni-spark-fill"></em><span>{{ __('Mark Active') }}</span></a></li>
                                        @endif

                                        @if(data_get($scheme, 'status') != SStatus::INACTIVE)
                                        <li><a href="javascript:void(0)" class="actn-upivs" data-action="{{ SStatus::INACTIVE }}" data-uid="{{ the_hash($scheme->id) }}"><em class="icon ni ni-spark-off-fill"></em><span>{{ __('Mark Inactive') }}</span></a></li>
                                        @endif

                                        @if(data_get($scheme, 'status') != SStatus::ARCHIVED)
                                        <li><a href="javascript:void(0)" class="actn-upivs" data-action="{{ SStatus::ARCHIVED }}" data-uid="{{ the_hash($scheme->id) }}"><em class="icon ni ni-archive-fill"></em><span>{{ __('Mark Archive') }}</span></a></li>
                                        @endif

                                        <li><a href="javascript:void(0)" class="actn-upivs" data-action="{{ SStatus::DELETE  }}" data-uid="{{ the_hash($scheme->id) }}"><em class="icon ni ni-trash-fill"></em><span>{{ __('Delete Scheme') }}</span></a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>

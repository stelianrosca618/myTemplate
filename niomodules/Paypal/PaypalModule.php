...
{
    const VERSION = '1.2.4';
    const LAST_UPDATE = '05242023';
    const MIN_APP_VER = '1.4.0';
    ...
    public function getNonDecimalCurrencies()
    {
        return data_get(config('modules.'.PaypalModule::SLUG), 'non_decimal_currency');
    }

    public function updateTnxAmount($currency, $amount)
    {
        return in_array($currency, $this->getNonDecimalCurrencies()) ? round($amount) : number_format($amount, 2, '.', '');
    }

    public function updateSessionData()
    {
        $payment = Session::get('deposit_details');
        if (!empty($payment)) {
            $payment['amount'] = round($payment['amount']);
            $payment['total'] = round(to_sum($payment['amount'], $payment['amount_fees']));
            $payment['base_amount'] = round(($payment['amount'] / $payment['exchange_rate']), 2);
            $payment['base_total'] = to_sum($payment['base_amount'], $payment['base_fees']);
            Session::put('deposit_details', $payment);
        }
        return $payment;
    }
    ...
    {
        $tnxCurrency = data_get($this->transaction, 'tnx_currency');
        $tnxTotal = data_get($this->transaction, 'tnx_total');
        ...
        $request->body = [
            ...
            "purchase_units" => [[
                ...
                "amount" => [
                    "value" => $this->updateTnxAmount($tnxCurrency, $tnxTotal),
                    "currency_code" => $tnxCurrency
                ]
            ]],
            "application_context" => [

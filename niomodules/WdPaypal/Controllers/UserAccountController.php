...
class UserAccountController extends Controller
{
    ...
    private function validateInput(Request $request, $update = false)
    {
        $input =  $request->validate([
            'wdm-label' => 'nullable|string',
            'wdm-email' => 'required|email',
            'wdm-currency' => ($update) ? 'nullable|string' : 'required|string',
        ], [
            'wdm-email.*' => __('Enter a valid email address.')
        ]);

        return array_map('strip_tags_map', $input);
    }
    ...
    public function save(Request $request)
    {
        ...
        $name = ($input['wdm-label']) ? $input['wdm-label'] : 'AC-'.substr(sprintf('%04s', auth()->user()->id), -4, 4).'-'.rand(1001, 9999);
    ...
    public function update($hash, Request $request)
    {
        ...
        $account = UserAccount::where('id', $id)

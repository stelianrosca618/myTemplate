@php 
	...
	$docs = data_get($setting, 'main');
	$fields = data_get($setting, 'field');
@endphp
...
<div class="nk-block-head nk-block-head-lg wide-xs mx-auto">
	<div class="nk-pps-steps">
		...
		@if (!empty(data_get($setting, 'alter')))
			<span class="step"></span>
		@endif
		<span class="step"></span>
	</div>
			...
			<div class="nk-kycfm">
				<div class="nk-kycfm-content">
					...
					<div class="row g-3">
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                <label class="form-label" for="country">{{ __("Issued by Country") }}</label>
                                <div class="form-control-group">
                                    <select class="form-control form-select" name="country" id="country" data-search="on" data-placeholder="{{ __('Choose country') }}" required>
                                        <option></option>
                                        @foreach($countries as $code => $country)
                                            <option value="{{ $country }}"{{ (data_get($userDoc, 'meta.country') == $country) ? ' selected' : '' }}>{{ config('countries')[$code] }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
						...
						<div class="col-md-{{ ((data_get($fields, 'expiry.show') != "on" && data_get($fields, 'issue.show') != "on") || (data_get($fields, 'expiry.show') == "on" && data_get($fields, 'issue.show') == "on")) ? '12' : '6' }}">
							<div class="form-group">
								<label class="form-label" for="doc-number">
									{{ __("Document Number") }}
									@if (data_get($fields, 'id.req') != "on")
										<small class="text-soft">{{ __("Optional") }}</small>
									@endif
								</label>
								<div class="form-control-group">
									<input type="text" name="number" value="{{ (data_get($userDoc, 'meta.number')) ? data_get($userDoc, 'meta.number') : '' }}" 
											id="doc-number" class="form-control"{{ (data_get($fields, 'id.req') == "on") ? ' required' : '' }}>
								</div>
							</div>
						</div>
						@endif
						...
						<div class="col-md-{{ (data_get($fields, 'id.show') != "on" && data_get($fields, 'expiry.show') != "on") ? '12' : '6' }}">
							<div class="form-group">
								<label class="form-label" for="issue-date">
									{{ __("Issue Date") }}
									@if (data_get($fields, 'issue.req') != "on")
										<small class="text-soft">{{ __("Optional") }}</small>
									@endif
								</label>
								<div class="form-control-group">
									<input type="text" name="issue" value="{{ (data_get($userDoc, 'meta.issue')) ? data_get($userDoc, 'meta.issue') : '' }}" 
											id="issue-date" class="form-control date-picker" data-date-start-date="-10y" data-date-end-date="-15d"{{ (data_get($fields, 'issue.req') == "on") ? ' required' : '' }}>
								</div>
							</div>
						</div>
						@endif
						...
						<div class="col-md-{{ (data_get($fields, 'id.show') != "on" && data_get($fields, 'issue.show') != "on") ? '12' : '6' }}">
							<div class="form-group">
								<label class="form-label" for="expiry-date">
									{{ __("Expiry Date") }}
									@if (data_get($fields, 'expiry.req') != "on")
										<small class="text-soft">{{ __("Optional") }}</small>
									@endif
								</label>
								<div class="form-control-group">
									<input type="text" name="expiry" value="{{ (data_get($userDoc, 'meta.expiry')) ? data_get($userDoc, 'meta.expiry') : '' }}" 
											id="expiry-date" class="form-control date-picker" data-date-start-date="+7d"{{ (data_get($fields, 'expiry.req') == "on") ? ' required' : '' }}>
								</div>
							</div>
						</div>
						@endif
					...
					<div class="nk-kycfm-note mt-4">
						...
						<p>{{ __("To avoid delays when verifying account, please make sure you have provide valid information.") }}</p>
					</div>
				</div>
			</div>
		</div>
		<div class="nk-kyc-app-action mt-4">
			<a class="btn btn-lg btn-wider btn-primary kyc-step-dcs" data-action="next"><span>{{ __("Click to Next") }}</span></a>
			<a href="javascript:void(0)" class="link link-primary link-btn btn-block mt-2 kyc-step-dcs" data-action="back"><span>{{ __("Back to Previous") }}</span></a>
		</div>
	</form>
</div>
<script>
	$('.kyc-step-dcs').on('click', function (e) {
		e.preventDefault();
		var $self = $(this), action = $self.data('action'), $form = $self.closest('form');
		var url = (action == 'next') ? "{{ route("user.kyc.verify.documents.update") }}" : "{{ route('user.kyc.verify.basic') }}", 
			data = (action == 'next') ? $form.serialize() : {confirm: true, method: action};
